...
OUT_DIR        = "plots/"
OUT_NORM_CSV   = os.path.join(OUT_DIR, "posterior_cm_normalized.csv")
OUT_PNG        = os.path.join(OUT_DIR, "posterior_dotmatrix_correct_vs_wrong_gemma.png")
...
# ===================== HELPERS ============================
def extract_raw_posterior(text: str) -> str:
    """Extract exact raw text after 'Posterior region:' up to the next period or end."""
    if pd.isna(text):
        return ""
...
df["posterior_gt_raw"]  = df[COL_GT].apply(extract_raw_posterior)
df["posterior_gen_raw"] = df[COL_PRED].apply(extract_raw_posterior)

filtered = df[
    (df["posterior_gt_raw"]  != "") &
    (df["posterior_gen_raw"] != "")
].copy()

# Crosstab (counts)
cm_counts = pd.crosstab(
    filtered["posterior_gt_raw"],
    filtered["posterior_gen_raw"],
    dropna=False
)
