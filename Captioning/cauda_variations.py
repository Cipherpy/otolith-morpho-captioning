...
def extract_raw_cauda(text):
    """Extract the exact raw text after 'Cauda:' up to the next period or end."""
    if pd.isna(text):
        return ""
...
df["cauda_gt_raw"]  = df["Description"].apply(extract_raw_cauda)
df["cauda_gen_raw"] = df["generated_caption"].apply(extract_raw_cauda)
...
conf = pd.crosstab(
    ...
    rownames=["Ground Truth Cauda"],
    colnames=["Predicted Cauda"],
    dropna=False
)
...
plt.tight_layout()
plt.savefig("llama/plots/cauda_confusion_matrix_seaborn.png", dpi=600, bbox_inches="tight")
plt.show()
