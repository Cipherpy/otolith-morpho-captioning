...
def extract_raw_ostium(text):
    ...
    if pd.isna(text):
        return ""
    m = re.search(r'Side:\s*([^\.]*)', str(text), flags=re.IGNORECASE)
    return m.group(1).strip() if m else ""

# ---- load & prep
df = pd.read_csv("/home/cmlre/Desktop/otolith_new/output_filtered.csv")

df["side_gt_raw"]  = df["Description"].apply(extract_raw_ostium)
df["side_gen_raw"] = df["generated_caption"].apply(extract_raw_ostium)

filtered = df[(df["side_gt_raw"] != "") & (df["side_gen_raw"] != "")]

conf = pd.crosstab(
    filtered["side_gt_raw"],
    filtered["side_gen_raw"],
    rownames=["Ground Truth Side"],
    colnames=["Predicted Side"],
    dropna=False
)
...
plt.tight_layout()
plt.savefig("llama/plots/llama_side_confusion_matrix_seaborn.png", dpi=600, bbox_inches="tight")
plt.show()