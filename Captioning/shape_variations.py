...
def extract_raw_shape(text):
    """Extract the exact raw text after 'Shape:' up to the next period or end."""
    if pd.isna(text):
        return ""
...
df["shape_gt_raw"]  = df["Description"].apply(extract_raw_shape)
df["shape_gen_raw"] = df["generated_caption"].apply(extract_raw_shape)

filtered = df[(df["shape_gt_raw"] != "") & (df["shape_gen_raw"] != "")]

conf = pd.crosstab(
    filtered["shape_gt_raw"],
    filtered["shape_gen_raw"],
    rownames=["Ground Truth Shape"],
    colnames=["Predicted Shape"],
    dropna=False
)
...
plt.tight_layout()
plt.savefig("plots/shape_confusion_matrix_seaborn.png", dpi=600, bbox_inches="tight")
plt.show()
